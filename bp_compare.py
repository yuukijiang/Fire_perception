...
batch_size = 3
data_train = bp.batch(x,3)
label_train = bp.batch(y,3)
data_test = bp.batch(x1,3)
label_test = bp.batch(y1,3)
...
model = bp.bp(input_size, hidden_size,output_size)
para = summary(model, (3,3))
print(para)
print(model)
...
#                       而且梯度变大意味着学习收敛速度快，能大大加快训练速度。
    for batch_idx, (data, target) in enumerate(zip_train):
        ...
        data = data.view(-1, input_channels)
        # if args.permute:
        #     data = data[:, :, permute]
...
if __name__ == "__main__":
    for epoch in range(1, epochs + 1):
        ...
        print(test_time_str)
        if epoch == 30:
            with open('E:\\fire_detection\\tcn\pth\\bp_result.txt', 'w') as f:
