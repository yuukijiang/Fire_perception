...
class TemporalConvNet2(nn.Module):
    def __init__(self, num_inputs, num_channels, kernel_size=2, dropout=0.2):
        ...
        for i in range(num_levels):
            ...
            in_channels = num_inputs if i == 0 else num_channels[i - 1]  # 确定每一层的输入通道数
            out_channels = num_channels[i]  # 确定每一层的输出通道数
            layers += [TemporalBlock(in_channels, out_channels, kernel_size *2, stride=1, dilation=dilation_size,
                                     padding=(kernel_size*2 - 1) * dilation_size, dropout=dropout)]

        self.network2 = nn.Sequential(*layers)   # *作用是将输入迭代器拆成一个个元素
...
class TCN(nn.Module):
    def __init__(self, input_size, output_size, num_channels, kernel_size, dropout):
        ...
        self.tcn2 = TemporalConvNet2(input_size, num_channels, kernel_size=kernel_size, dropout=dropout)
    ...
    def forward(self, inputs):
        ...
        a = self.linear(y1[:, :, -1])  # 增加一个维度,1D·FCN
