...
# 数据迭代器，在utils.py里面
data_train, label_train = read_data_pro.read_txt(path_train,window = 20 ,stride = 10,batch_size=batch_size)
data_test, label_test = read_data_pro.read_txt(path_test,window = 20 ,stride = 10 ,batch_size=batch_size)
...
# 定义卷积核大小
model = tcn_pro.TCN(input_channels, n_classes, channel_sizes, kernel_size=kernel_size, dropout=args.dropout)
para = summary(model, (6, 20))
print(model)
print(para)
if args.cuda:
...
#                       而且梯度变大意味着学习收敛速度快，能大大加快训练速度。
    for batch_idx, (data, target) in enumerate(zip_train):
        ...
        output,a = model(data)
        for i in a:
            svm_train_data_block = []
            for j in i:
                svm_train_data_block.append(str(j.detach().numpy()))
                svm_train_data_block.append(' ')
            svm_train_data.append(svm_train_data_block)
        for i in target:
            svm_train_label.append(str(i.long().numpy()))
...
#   每一个批次做完之后就要把train_loss的值给清零，以供下一epoch继续计算
    return svm_train_data, svm_train_label
...
#   测试误差，指在测试集上产生的误差
    ...
    with torch.no_grad():
        ...
        # GPU相关操作
            ...
            output,b = model(data)
            # gai = torch.exp(output)
            for i in range(len(b)):
                svm_test_data_block = []
                for j in b[i]:
                    svm_test_data_block.append(str(j.numpy()))
                    svm_test_data_block.append(' ')
                svm_test_data.append(svm_test_data_block)
            for i in target:
                svm_test_label.append(str(i.long().numpy()))
        ...
        print(cm)
        acc = float(100 * correct) / (length_test * batch_size)
        print('\nTest set: Average loss: {:.4f}, Accuracy: {}/{} ({:.4f}%)\n'.format(
            test_loss, correct, length_test * batch_size,
            acc))
        return acc, svm_test_data, svm_test_label,np.array(targets).squeeze(),np.array(preds).squeeze()
...
if __name__ == "__main__":
    ...
    for epoch in range(1, epochs + 1):
        ...
        svm_train_data, svm_train_label = train(epoch)
        train_end = time.time()
        ...
        accy, svm_test_data, svm_test_label, test_targets, test_preds = test()
        test_end = time.time()
        test_time_str = "test run time: %.4f seconds" % (test_end - test_start)
        print(test_time_str)
        svm_train = zip(svm_train_data, svm_train_label)
        svm_test = zip(svm_test_data, svm_test_label)
        if(accy > 99 or epoch == 30):
            # with open('E:\\fire_detection\\tcn\pth\\tcn_app_svm_result.txt', 'w') as f:
            #     for a, b in zip(test_targets, test_preds):
            #         f.write(str(a)+ ' ' + str(b) + "\r")
            print(all_mean_loss)
            with open('E:\\fire_detection\\tcn\pth\svm_train.txt', 'w') as f:
                for a, b in svm_train:
                    for c in a:
                        f.write(c)
                    f.write(b + "\r")
            with open('E:\\fire_detection\\tcn\pth\svm_test.txt', 'w') as f:
                for a, b in svm_test:
                    for c in a:
                        f.write(c)
                    f.write(b + "\r")
