...
def read_txt(path,window,stride,batch_size):
    ...
    for filename in files:  #遍历文件夹里的每一个txt
        ...
        for i in f.readlines():
            j = i.split(' ')
            data_line.append([float(j[0]), float(j[1]), float(j[2]), float(j[3]), float(j[4]), float(j[5])])   #取出三种传感器数据
    ...
    for i in label:
        for j in i:
            if(j == 0):
                label_0_block = label_0_block + 1
            elif(j == 1):
                label_1_block = label_1_block + 1
            elif(j == 2):
                label_2_block = label_2_block + 1
        label_0.append(label_0_block)
        label_1.append(label_1_block)
        label_2.append(label_2_block)
        label_0_block = 0
        label_1_block = 0
        label_2_block = 0
...
if __name__ == '__main__':
    print(1)
    path_train = 'E:\\fire_detection\\tcn\pth\mk_process_data\\train'
    label0 = 0
    label1 = 0
    label2 = 0
